...
from ..constants import BUILT_IN_DATASET_PATH, TEST_DATA_OPTION

# from utils.exceptions import InvalidFileError


def get_data():
    print("The address of the files:", BUILT_IN_DATASET_PATH)
    items = os.listdir(BUILT_IN_DATASET_PATH)

    # 遍历所有项（文件和子文件夹）
    for file in items:
        item_path = os.path.join(BUILT_IN_DATASET_PATH, file)

        if os.path.isfile(item_path):  # 判断是否为文件
            if file.endswith(('.csv', '.xlsx', '.xls')):
                TEST_DATA_OPTION.append(file)
    # # 遍历文件夹
    # for root, dirs, files in os.walk(BUILT_IN_DATASET_PATH):
    #     for file in files:
    #         # 检查文件是否为表格文件（如CSV、Excel等）
    #         if file.endswith(('.csv', '.xlsx', '.xls')):
    #             TEST_DATA_OPTION.append(file)
    if TEST_DATA_OPTION:
        monitor = True
